import { countByString } from '../util';

type SeatKey = 'L' | '.' | '#';

const part1 = (seats: SeatKey[][]) => {
  let activeKey: string = flat2(seats);
  let curKey = '';
  let steps = 0;
  while (activeKey !== curKey) {
    const seatsCopy = seats.map((arr) => arr.slice());
    steps++;
    activeKey = curKey;
    for (let i = 0; i < seatsCopy.length; i++) {
      for (let j = 0; j < seatsCopy[0].length; j++) {
        if (seatsCopy[i][j] === '.') continue;
        const neighbors = countNeighbors(seatsCopy, i, j);
        if (neighbors === 0 && seatsCopy[i][j] === 'L') {
          seats[i][j] = '#';
        } else if (neighbors >= 4 && seatsCopy[i][j] === '#') {
          seats[i][j] = 'L';
        }
      }
    }
  ...
  return countByString(flat2(seats))['#'];
};

const countNeighbors = (grid: SeatKey[][], y: number, x: number) => {
  let c = 0;

  if (y > 0 && x > 0 && grid[y - 1][x - 1] === '#') c++;
  if (y > 0 && grid[y - 1][x] === '#') c++;
  if (y > 0 && x < grid[0].length - 1 && grid[y - 1][x + 1] === '#') c++;
  if (x > 0 && grid[y][x - 1] === '#') c++;
  if (x < grid[0].length - 1 && grid[y][x + 1] === '#') c++;
  if (y < grid.length - 1 && x > 0 && grid[y + 1][x - 1] === '#') c++;
...
const flat2 = (arr: any[][]) => {
  return arr.reduce((a, c) => `${a}${c.reduce((a, c) => `${a}${c}`, '')}`, '');
};

const day11test: SeatKey[][] = `L.LL.LL.LL
LLLLLLL.LL
L.L.L..L..
LLLL.LL.LL
L.LL.LL.LL
...
L.LLLLL.LL`
  .split('\n')
  .map((r) => r.split('')) as SeatKey[][];

const day11inp: SeatKey[][] = `.LLLL.L.LLLL.LL.LLL.L.LLLLLLL.LLLLL.LLLLLLLLLLLLL.LLLL.LLL.LLLLL.LLLLLLLL.LLLLL.LLLLLLL.LLLLL
