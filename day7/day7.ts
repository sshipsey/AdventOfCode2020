const day7 = (rules: string[]) => {
  const bagMap: { [bag: string]: string[] } = {};
  for (let rule of rules) {
    const match = rule.match(/\w+ \w+/);
    const bag = match && match[0];
    const innerBagRgx = /(\d+) (\w+ \w+)(?= bags?(?:(?:, )|\.))/g;
    let innerBags: string[] = [];
    let m;
    while ((m = innerBagRgx.exec(rule))) {
      innerBags = innerBags.concat(m[2]);
    }
    bagMap[bag!] = innerBags;
  }
  let ptr = 0;
  let bags = ['shiny gold'];

  while (ptr < bags.length) {
    for (let outerBag of Object.keys(bagMap)) {
      if (bagMap[outerBag].includes(bags[ptr])) {
        bags = bags.concat(outerBag);
      }
    }
    ptr++;
  }
  return new Set<string>(bags).size - 1;
};

let total = 0;
const day7p2 = (rules: string[]) => {
  const bagRules = bagMap(rules);
  getVal(bagRules, 'shiny gold', 1);
  return total;
};

const getVal = (
  map: { [bag: string]: { [bag: string]: number } },
  bag: string,
  count: number
): any => {
  for (let b of Object.entries(map[bag])) {
    total += b[1] * count;
    getVal(map, b[0], b[1] * count);
  }
};

const bagMap = (rules: string[]) => {
  const bagMap: { [bag: string]: { [bag: string]: number } } = {};
  for (let rule of rules) {
    const match = rule.match(/\w+ \w+/);
    const bag = match && match[0];
    const innerBagRgx = /(\d+) (\w+ \w+)(?= bags?(?:(?:, )|\.))/g;
    let innerBags: { [bag: string]: number } = {};
    let m;
    while ((m = innerBagRgx.exec(rule))) {
      innerBags[m[2]] = +m[1];
    }
...
const day7inp = `wavy bronze bags contain 5 striped gold bags, 5 light tomato bags.
...
const day7test = `light red bags contain 1 bright white bag, 2 muted yellow bags.
...
console.log(day7(day7inp.split('\n')));
console.log(day7p2(day7inp.split('\n')));
