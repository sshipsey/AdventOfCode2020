const byr = (v: string) => +v >= 1920 && +v <= 2002;
const iyr = (v: string) => +v >= 2010 && +v <= 2020;
const eyr = (v: string) => +v >= 2020 && +v <= 2030;
const hgt = (v: string) => {
  const heightParams: Record<string, { max: number; min: number }> = {
    cm: {
      max: 193,
      min: 150,
    },
...
const day4 = (input: string) => {
  ...
  let lookup: { [key: string]: (v: string) => boolean } = {
    byr,
    iyr,
    eyr,
    hgt,
  ...
  return passports
    .map((passport) =>
      Object.keys(lookup)
        .map((key) => {
          const val = getKeyValue(passport, key);
          return val !== null && lookup[key](val);
        })
        .every((v) => v)
        ? 1
        : 0
    )
    .reduce((a: number, b: 1 | 0) => a + b, 0);
};

const day4inp = `pid:827837505 byr:1976
hgt:187cm
iyr:2016
hcl:#fffffd
eyr:2024
