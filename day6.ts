...
const day6p1 = (groups: string[]) =>
  groups
    .map(stripCr)
    .map(
      (str) => new Set<string>([...str]).size
    )
    .reduce(sum, 0);

const day6p2 = (groups: string[]) =>
  flat(
    groups.map((group) => {
      const groupSize = group.split('\n').length;
      const concatGroup = stripCr(group);
      return Object.values(countByString(concatGroup)).map((count) =>
        count === groupSize ? 1 : 0
      );
    })
  ).reduce(sum, 0);
...
const groups = day6inp.split('\n\n');

console.log(day6p1(groups));
console.log(day6p2(groups));
